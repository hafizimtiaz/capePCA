...
for itr = 1:nensemble
    for n_id = 1:length(N_all)
        ...
        en_dist_djica_ndp(itr, n_id) = trace(U_dist_hafiz_ndp' * A * U_dist_hafiz_ndp);
        en_dist_djica_dp(itr, n_id) = trace(U_dist_hafiz_dp' * A * U_dist_hafiz_dp);
        en_loc_djica_ndp(itr, n_id) = trace(U_loc_djica_ndp' * A * U_loc_djica_ndp);
        en_loc_djica_dp(itr, n_id) = trace(U_loc_djica_dp' * A * U_loc_djica_dp);

    end
end
save('mnist_vs_samples', 'N_all', 'epsilon', 'delta', 'en_pool_ndp', 'en_pool_dp', 'en_dist_hafiz_ndp', 'en_dist_hafiz_dp', 'en_dist_djica_ndp', 'en_dist_djica_dp', 'en_loc_hafiz_ndp', 'en_loc_hafiz_dp', 'en_loc_djica_ndp', 'en_loc_djica_dp')